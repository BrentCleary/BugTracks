    ...
    public static class DataUtility
    {
        ...
        public static async Task SeedDefaultTicketStatusAsync(ApplicationDbContext context)
        {
            try
            {
                ...
                var dbTicketStatuses = context.TicketsStatuses.Select(c => c.Name).ToList();
                await context.TicketsStatuses.AddRangeAsync(ticketStatuses.Where(c => !dbTicketStatuses.Contains(c.Name)));
                await context.SaveChangesAsync();
        ...
        public static async Task SeedDefautTicketsAsync(ApplicationDbContext context)
        {
            ...
            int statusNew = context.TicketsStatuses.FirstOrDefault(p => p.Name == BTTicketStatus.New.ToString()).Id;
            int statusDev = context.TicketsStatuses.FirstOrDefault(p => p.Name == BTTicketStatus.Development.ToString()).Id;
            int statusTest = context.TicketsStatuses.FirstOrDefault(p => p.Name == BTTicketStatus.Testing.ToString()).Id;
            int statusResolved = context.TicketsStatuses.FirstOrDefault(p => p.Name == BTTicketStatus.Resolved.ToString()).Id;
